...
CREATE TABLE IF NOT EXISTS `Insurance Company`.`claims` (
   ...
   FOREIGN KEY (`plan`)
   REFERENCES `Insurance Company`.`plans`(`plan_ID`),
   FOREIGN KEY (`beneficiary`, `plan`)
   REFERENCES `Insurance Company`.`dependents`(`name`, `plan`),
   check (`state` in ('resolved', 'unresolved'))
   );
   

-- we assumed that there are only 9000 plans we will sell so they will take numbers between 1001 : 10000.
ALTER TABLE `Insurance Company`.`plans` AUTO_INCREMENT = 1001;
